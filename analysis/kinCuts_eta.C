// Macro to fix the small difference obtained in kinCuts_cs2.C relative to add_cs_function.C
// Please change and remove everything you want.
// This was originally a copy of add_cs_function.C, which holds the results we want to replicate.
...
TString output_name = "kinematic_cuts_eta"; // Output name of Fig. 1 reproduction. Plots <pT>(Nch) for n_centralities for both collision energies.
...
TH1D* get_pT_TH1Dhistogram(const std::string& filename, double EHFmin = 0.0, double EHFmax = 250.0, double low_eta = 0.0, double high_eta = 0.0);
TH1D* Hagedorn_extrapolation(TH1D* hist_pT, const std::string& filename, double EHFmin = 0.0, double EHFmax = 250.0, int eta_index = 0);
void plot_graphs();
...
void kinCuts_eta(){
    ...
    for(int k = 0; k < Eta_cuts.size(); ++k){
        ...
        for(int i = 0; i < n_centralities; ++i){
            ...
            hist_5 = Hagedorn_extrapolation(get_pT_TH1Dhistogram(dataFile_5TeV.path, low5, high5, low_eta, high_eta), dataFile_5TeV.path, low5, high5, k);
            ...
            hist_8 = Hagedorn_extrapolation(get_pT_TH1Dhistogram(dataFile_8TeV.path, low8, high8, low_eta, high_eta), dataFile_8TeV.path, low8, high8, k);
...
TH1D* get_pT_TH1Dhistogram(const std::string& filename, double EHFmin, double EHFmax, double low_eta, double high_eta){
...
TH1D* Hagedorn_extrapolation(TH1D* hist_pT, const std::string& filename, double EHFmin, double EHFmax, int eta_index){
    ...
    static int eta_counter = 0;

    // Original.
    pT_fit = new TF1(Form("ptfit_%d_%d_%d",cc_low,cc_up, eta_counter++),"[0]*x*pow(1.+1./sqrt(1.-[1]*[1])*(sqrt(x*x+[4]*[4])-x*[1])/[3]/[2],-[3])",0.,upper_pt_forFit);
    if(eta_index == 0) pT_fit->SetParameters(75000000000.,0.3,0.1,6.,0.14);//We used these values for initialization        
    else pT_fit->SetParameters(7500000000.,0.3,0.1,6.,0.14);//We used these values for initialization
...
void plot_graphs(){
    ...
    int colors[6] = {kBlack, kRed, kBlue, kOrange, kGreen-2, kMagenta-2};
    ...
    //graphs[0]->GetXaxis()->SetNdivisions(505, kTRUE);
    //graphs[0]->GetYaxis()->SetNdivisions(505, kTRUE);
    //graphs[0]->GetXaxis()->SetLimits(212, 260);
    //graphs[0]->GetYaxis()->SetRangeUser(0.16, 0.36);
    graphs[0]->GetXaxis()->SetNdivisions(505, kTRUE);  // 5 major divisions = 50/5 = 10 units each
    graphs[0]->GetXaxis()->SetLimits(140, 320);
    ...
    auto leg = new TLegend(0.68,0.58,0.88,0.88);
    leg->SetBorderSize(0);
    leg->SetFillStyle(0);
    ...
    leg->AddEntry(graphs[5],"|#eta| < 0.3","lep");
    ...
    draw_CMS_Header("p_{T} > 0 GeV", 0.16, 0.74, 0.038, 11);
